...
void BuoyantObject::ApplyBuoyancyForce()
{
  ...
  // Split the link into separate buoyant objects (or bobj)
  double nbBObj=11; // user defined (strictly positive odd integer)
  
  // Make sure that nbBObj is a strictly positive odd integer (for implementation reasons)
  if (int(nbBObj) <0)
  {
    nbBObj = -int(nbBObj);
  }
  ...
  // Create a list of vectors "CoG -> Top of bobj bounding box" in the Gazebo body-fixed frame
  double boxH = this->boundingBox.ZLength(); // height of the bounding box
  double boxL = this->boundingBox.YLength(); // length of the bounding box
  double theta_max = -4*M_PI/180; // pitch angle from which the furthest back bobj is outside the water (rad) - user defined
  double radius = sqrt(boxL*boxL + 2*sin(theta_max)*boxL*boxH + boxH*boxH)/(2*cos(theta_max)); // the distance between the CoG and the furthest top of bobj, located at x= +/- L/2
  double offsetH = boxH/2 - sqrt(radius*radius - (boxL/2)*(boxL/2)); // height offset on virtual "top of bobj bounding box" to let the sub dive (and not be stuck to the surface)
  double interval = boxL/(nbBObj-1);// spacing between objects
  std::list<ignition::math::Vector3d> bObjBodyList;
  for (int bObjCounter=0;bObjCounter<nbBObj;bObjCounter++)
  {
    bObjBodyList.push_back(ignition::math::Vector3d(-boxL/2 + bObjCounter*interval,0,boxH/2-offsetH));
  }

  // Get the orientation of the link in the ENU inertial reference frame
  double linkRoll = pose.Rot().Roll();
  double linkPitch = pose.Rot().Pitch();
  double linkYaw = pose.Rot().Yaw();
  
  // Express the bObj vectors in the ENU inertial reference frame
  std::list<ignition::math::Vector3d> bObjInertialList;  
  auto rotX = ignition::math::Matrix3d(1,0,0,
                                       0,cos(linkRoll),-sin(linkRoll),
                                       0,sin(linkRoll),cos(linkRoll));
  auto rotY = ignition::math::Matrix3d(cos(linkPitch),0,sin(linkPitch),
                                       0,1,0,
                                       -sin(linkPitch),0,cos(linkPitch));
  auto rotZ = ignition::math::Matrix3d(cos(linkYaw),-sin(linkYaw),0,
                                       sin(linkYaw),cos(linkYaw),0,
                                       0,0,1);                       
  for (ignition::math::Vector3d bObjVec : bObjBodyList)
  {
    bObjInertialList.push_back(rotZ*rotY*rotX*bObjVec);
  }
  
  // Evaluate which bobj are fully submerged
  std::list<int> bObjWeightList; // Weight list to know which bobj is fully submerged
  for (ignition::math::Vector3d bObjVec : bObjInertialList)
  {
    if (pose.Pos().Z() + bObjVec.Z() > 0) // case not fully submerged
    {
        bObjWeightList.push_back(0);
    }
    else // case fully submerged
    {
        bObjWeightList.push_back(1);
    }
  }

  // Determine the barycentre of bobj that are fully submerged
  auto sumbObjCoord=ignition::math::Vector3d(0,0,0);
  auto nbObjUnderwater = 0;
  std::list<int>::iterator itBObjWeightList = bObjWeightList.begin();
  for (ignition::math::Vector3d bObjVec : bObjBodyList)
  {
    auto weight = *itBObjWeightList;
    sumbObjCoord=sumbObjCoord + bObjVec * weight;
    nbObjUnderwater = nbObjUnderwater + weight;
    std::advance(itBObjWeightList, 1);
  }
  
  auto bObjBarycentre = ignition::math::Vector3d(0,0,0);
  if (nbObjUnderwater>0) // Avoiding the NAN case
  {
    bObjBarycentre = sumbObjCoord/nbObjUnderwater;
  }
    
  // Weigh the Barycentre closer to the centre, to reduce the maximum torque generated by the shifted buoyancy force
  // The front has more impact than the back of the sub (for geometry reasons) in terms of buoyancy loss, which means that the shiftedCoB will be further from the
  // centre along the x axis if the X component of the barycentre is negative
  // The default weight is 1.0 and means that no weighting has been applied. If weighting is < 1.0 then the effect of the buoyancy force is reduced.
  double frontWeight = 1.0; // user defined
  double backWeight = frontWeight/100000; // user defined
  if(bObjBarycentre.X()>=0)
  {
    bObjBarycentre = bObjBarycentre * backWeight;
  }
  else
  {
    bObjBarycentre = bObjBarycentre * frontWeight;
  }
  
  // Determine the shifted CoB location in the Gazebo body-fixed frame
  auto CoBCoord=this->GetCoB();
  auto shiftedCoBCoord = ignition::math::Vector3d(bObjBarycentre.X(),bObjBarycentre.Y(),CoBCoord.Z());

  if (!this->isSurfaceVessel)
  {
#if GAZEBO_MAJOR_VERSION >= 8
        this->link->AddForceAtRelativePosition(buoyancyForce, shiftedCoBCoord);
#else
        this->link->AddForceAtRelativePosition(
          math::Vector3(buoyancyForce.X(), buoyancyForce.Y(), buoyancyForce.Z()),
          math::Vector3(shiftedCoBCoord.X(), shiftedCoBCoord.Y(), shiftedCoBCoord.Z()));
#endif
  }
  else
  {
